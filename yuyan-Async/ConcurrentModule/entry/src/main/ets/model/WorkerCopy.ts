...
workerPort.onmessage = (message): void => {
  ...
  for (let i = 0; i < fileNames.length; i++) {
    ...
    try {
      ...
      let countTest = fileIo.listFileSync(message.data.destDir).length;
  ...
  try {
    let count = fileIo.listFileSync(message.data.destDir).length;
    let listFiles = fileIo.listFileSync(message.data.destDir);
